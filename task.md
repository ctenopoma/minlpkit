...
※ ML(GNN/学習分岐)・LLM・GPU(cuOpt等)はハードウェア/基盤の導入可否判断が必要なため改善候補から除外。
...
## Phase 10: 可視化アップデート(実験管理=最適化MLOps)— 着手
...
- [ ] **C2+A: runs一覧テーブルUI + UI刷新**(Sonnet): サイドバー=run一覧(条件/結果でソート・フィルタ)、
  選択runの重ね描き比較、共通シェルでデザイン統一
- [ ] **B: ライブ診断**: 求解中に診断ルールを逐次評価し症状バナー表示。Primal Integral/TTFFタイル
- [ ] **C3: スイープ実行**: 設定リスト/グリッドの一括実行ランナー(yaml, extras)+ parallel coordinates。
  rerun <run_id>(記録条件からの再現実行)

## スコープ外

- **ML/GNN系(Forge、学習分岐等)・LLM支援・GPU(cuOpt等)**: ハードウェア/基盤の導入可否判断が必要なため当面除外。診断ルール表の将来拡張候補として名前だけ残す
