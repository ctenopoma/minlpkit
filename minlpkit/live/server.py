...
from ..gpu import _DEFAULT_CUOPT_CMD, cuopt_available
...
@app.route("/api/gpu")
def gpu_status() -> Response:
    """GPU(cuOpt)機能の利用可否と起動コマンド要約を返す。

    ``mk.cuopt_available()`` の結果(WSL2/GPUのcuOpt CLIが起動可能か)を、既定の
    起動コマンドの要約とともに返す。ヘッダのGPU状態インジケータと、GPU実験(engine=
    cuopt/hybrid)runの文脈表示に使う。cuOptは完全に任意なので、未導入でも200を返し
    ``available: false`` とするだけでエラーにはしない(判定は結果をプロセス内キャッシュ)。
    """
    cmd = list(_DEFAULT_CUOPT_CMD)
    return jsonify({
        "available": cuopt_available(cmd),
        "cuopt_cmd": " ".join(cmd),
        "distro": cmd[2] if len(cmd) > 2 and cmd[0] == "wsl" else None,
    })
