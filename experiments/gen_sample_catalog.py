...
各行から GitHub ソース(main ブランチ)へリンク。事業ストーリーが特に詳しいサンプルには ⭐ を付す。
...
# 事業ストーリーが特に詳しいサンプル(⭐ を付す)。カテゴリ横断の一覧性のため明示的に列挙する。
FLAGSHIP = {
...
def write_category_page(cat: str, samples: list[SampleInfo]) -> Path:
    ...
    lines.append(
        f"**{len(samples)} 本**"
        + (f"(うち事業ストーリーが特に詳しいもの ⭐ {flag_n} 本)" if flag_n else "")
...
def write_index(by_cat: dict[str, list[SampleInfo]]) -> Path:
    ...
    lines.append("")
    lines.append(
        f"⭐ マークは事業ストーリーが特に詳しいサンプル({flag_total} 本)を示す。"
        "そのうち一部は、事業ストーリー→素朴な定式化→診断→改善を1本の物語として"
        "実行できる学習用 notebook(`docs/notebooks/samples/`)が付属する"
        "(下表で「notebook」リンクがあるものが該当)。")
    lines.append("")
    lines.append("## カテゴリ一覧")
    lines.append("")
    lines.append("| カテゴリ | 本数 | ⭐ | 概要 |")
    lines.append("| --- | :---: | :---: | --- |")
    ...
    lines.append("## 事業ストーリーが特に詳しいサンプル(⭐)")
    lines.append("")
    lines.append(
        "事業ストーリー→素朴な定式化→診断→改善を1本の物語として追える題材。"
        "「notebook」リンクがあるものは、その物語をそのまま実行できる学習用 notebook が付属する。")
    lines.append("")
