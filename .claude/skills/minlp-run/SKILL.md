...
| キー | ファイル | 特性 | 推奨時間制限 |
| --- | --- | --- | --- |
| `uc` | samples/unit_commitment.py | 4秒/1ノードで解ける(ルート集中型。トラジェクトリは短い) | 60s |
| `sched` | samples/scheduling.py | 1秒未満で解ける(ベースライン) | 60s |
| `plant` | samples/scheduling_plant.py | gap停滞が観察できる難問(300sでgap72%) | 120-300s |
