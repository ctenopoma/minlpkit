...
このページはインストール手順とマニュアル全体の構成を示す。**列生成・ベンダーズ・再定式化などの
手法を知らない場合**は、このマニュアル(API仕様の網羅)より先に
[プレイブック(症状→打ち手)](../playbook/index.md) を読むほうが早い。症状から該当する手法へ
直接ジャンプでき、「なぜ効くか」「どのくらい効くか」を実測付きで説明している。プレイブックの
各ページからは、原理・効果をグラフで確認できる手法notebookにもリンクしている。

診断エンジンを全サンプルに一括適用した棚卸しは [診断センサス](../census.md) にある。
