...
| --- | --- | :---: | :---: |
| cash_flow_matching | キャッシュフローマッチング債券選択問題 (Cash Flow Matching Problem) — 年金基金や保険会社の資産運用担当者が、将来の年限ごとに確定している負債支払い | — | [source](https://github.com/ctenopoma/minlpkit/blob/main/samples/finance_and_pricing/cash_flow_matching.py) |
...
| portfolio_mip | カーディナリティ制約付きポートフォリオ最適化 (Portfolio Optimization with Cardinality Constraints) — 資産運用会社のポートフォリオマネージャーが、複数の候補銘柄(株式・債券・REIT等)の | — | [source](https://github.com/ctenopoma/minlpkit/blob/main/samples/finance_and_pricing/portfolio_mip.py) |
