...
```mermaid
flowchart LR
  M["主問題<br>$$\min c^\top y + \eta$$<br>s.t. 蓄積カット"] -->|"$$\hat y$$ を固定"| S["サブ問題<br>$$\min Q(\hat y)$$<br>(運用費・通常LP)"]
  S -->|"双対から最適性カット<br>$$\eta \ge Q(\hat y) + u^\top(y - \hat y)$$"| M
  M -.->|"下界 (LB)"| CONV{"LB = UB?"}
  S -.->|"上界 (UB)"| CONV
  CONV -->|"収束"| DONE([最適])
  CONV -->|"未収束"| M
...
[ベンダーズ分解](../notebooks/improve/05_benders.ipynb) を参照。

### 効かないとき・注意

- 構造(結合制約が少数、主問題とサブ問題が分離できる)が前提。[診断ベンチマーク](../census.md)では
