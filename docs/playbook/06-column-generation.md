...
- 列生成という言葉は知られているが、具体的な実装方法までは示されていないことが多い。
...
制限主問題(現在持っている列 $p \in P'$ だけを使った LP)
...
を解いて双対 $\pi$ を得て、その $\pi$ のもとで「被約コストが負(価値がある)」
――すなわち $1 - \pi^\top a_p < 0$ となる――列を1本だけ
...
```mermaid
flowchart LR
  RMP["制限主問題 (RMP)<br>$$\min \sum_p \lambda_p$$<br>$$\text{s.t.} \sum_p a_p \lambda_p \ge d$$"] -->|"双対 $$\pi$$"| PRICE["pricing問題<br>$$\max \sum_i \pi_i a_i$$<br>s.t. パターン制約"]
  PRICE -->|"被約コスト $$1 - \pi^\top a \lt 0$$<br>の新しい列を追加"| RMP
  PRICE -.->|"$$1 - \pi^\top a \ge 0$$<br>(改善列なし)"| DONE([LP最適])
...
8反復で収束する(FINDINGS §3、[`colgen.html`](../gallery/colgen.html))。**留意点**: この
...
[列生成](../notebooks/improve/06_column_generation.ipynb) を参照。

## 双対安定化(Wentges)

退化問題では双対 $\pi$ が反復ごとに大きく振動し(tailing-off)、収束が遅くなる。
安定化中心 $\pi_{\text{center}}$(最良Farley下界の双対)へ

$$
\tilde\pi = \alpha\,\pi_{\text{center}} + (1-\alpha)\,\pi
$$

で平滑化してから pricing に渡すと振動が抑えられる。退化 cutting stock(17品目)で
反復 **31→25(−19%)**、LP境界382.75は不変
(FINDINGS §3、[`stabilize.html`](../gallery/stabilize.html))。$\alpha$ 過大(0.9)は
過剰安定化で未収束になる。

## price-and-branch

列生成後、生成済みの列だけを使って整数制限主問題を解く。**これは上界のみ**
(得られる値を $z_{\mathrm{PB}}$、真の整数最適を $z^\star$ とすると
$z_{\mathrm{PB}} \ge z^\star$ が言えるだけで、等号は保証されない)。厳密な整数最適を保証する
